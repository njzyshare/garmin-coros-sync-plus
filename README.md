...
| 3️⃣ | **佳明平台去重** | 佳明 API 会返回 `DUPLICATE_ACTIVITY`，不会重复导入 |

> sync_log.db 通过 GitHub Actions Artifact 在所有工作流之间共享传递。如果 Artifact 丢失（例如首次运行或缓存过期），某些活动可能会被重复同步。但 **is_sync 标记在各自独立的 DB 文件中不受影响**，加上佳明侧的去重兜底，整体机制是安全的。运行几个周期后防重标记会自动恢复。
