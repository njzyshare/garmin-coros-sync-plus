...
> ⚠️ **已知问题：爬升数据偏差**
>
> 高驰手表记录的运动会同步到佳明，但佳明收到 FIT 文件后会用自己的海拔纠正算法（DEM 数字高程模型）重新计算爬升，**不会直接使用高驰 FIT 文件中的爬升数据**。这导致佳明上显示的爬升数据明显偏高。
>
> **实际数据对比**（高驰 vs 佳明）：
> | 运动 | 距离 | 高驰爬升 | 佳明爬升 | 偏差 |
> |:----|:----:|:--------:|:--------:|:----:|
> | 跑步 6/28 | 15.42km | 173m | 372m | 2.15x |
> | 跑步 6/13 | 28.19km | 359m | 843m | 2.35x |
> | 徒步 6/28 | 6.72km | 530m | 1891m | 3.57x |
>
> 验证：佳明原生活动（直接用手表记录）的 `elevationCorrected=False`，爬升数据准确（21.72km 半马爬升仅 23m）。而高驰同步过来的活动 `elevationCorrected=True`，爬升被 DEM 模型放大。**这是佳明服务器端行为，代码层面无法控制。建议以高驰 App 中的爬升数据为准。**
