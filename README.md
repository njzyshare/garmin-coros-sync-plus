...
> - ✅ 佳明国内国际互传：同平台，数据格式一致
> - ❌ **高驰→佳明**：高驰手表记录的运动同步到佳明后，爬升数据可能会出现明显偏差。从 API 返回的数据看，高驰同步过来的活动 `elevationCorrected=True`，而佳明手表原生活动 `elevationCorrected=False`，两者在这个字段上不一致。具体数值上，6.72km 徒步高驰 FIT 中原始 `total_ascent=530m`，到佳明后 `elevationGain=1891m`。
>
> **实际数据对比**：
