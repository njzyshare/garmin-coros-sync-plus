...
> ⚠️ **已知问题：爬升数据偏差**
>
> 该问题**只会发生在高驰原生活动同步到佳明CN的方向**（coros-sync-garmin 工作流）。其他方向不受影响：
> - ✅ 佳明原生活动同步到高驰：高驰直接读取 FIT 文件中的原始爬升数据
> - ✅ 佳明国内国际互传：同平台，数据格式一致
> - ❌ **高驰→佳明**：高驰手表记录的运动同步到佳明后，爬升数据可能会出现明显偏差。从佳明 API 返回的数据看，高驰同步到佳明的活动 `elevationCorrected=True`，而佳明手表原生活动 `elevationCorrected=False`。从字段命名和值上来看，佳明是会对自己手表以外的活动做海拔校正，用自己的算法重新算一遍，结果导致高驰本来处理好的数据被佳明再加工后反而不准了。具体数值上，6.72km 徒步高驰 FIT 中原始 `total_ascent=530m`，到佳明后 `elevationGain=1891m`。
>
> **实际数据对比**：
> | 方向 | 运动 | 距离 | 源平台爬升 | 目标平台爬升 | 偏差 |
> |:---:|:----|:----:|:---------:|:----------:|:----:|
> | 高驰→佳明 | 跑步 | 15.42km | **173m**（高驰）| **372m**（佳明）| 2.15x |
> | 高驰→佳明 | 跑步 | 28.19km | **359m**（高驰）| **843m**（佳明）| 2.35x |
> | 高驰→佳明 | 徒步 | 6.72km | **530m**（高驰）| **1891m**（佳明）| 3.57x |
> | 佳明→高驰 | 半马 | 21.72km | **23m**（佳明）| **23m**（高驰）| 1.0x ✅ |
> | 佳明→佳明 | 徒步 | 6.6km | **102m**（CN）| **102m**（INTL）| 1.0x ✅ |
>
> 可以看到佳明→高驰方向爬升完全一致（0 偏差），佳明国内国际互传也完全一致，而高驰→佳明方向偏差 2~3.5 倍。目前看来，这是佳明服务器端行为，暂时我也木的办法解决 ┑(￣Д ￣)┍，这种情况以高驰 App 中的爬升数据为准。

### 关于运动类型映射的说明

佳明和高驰使用不同的运动类型编码体系。佳明手表记录的原地踏步/短距离活动（如 12m、62m），通过 `garmin-coros-sync` 同步到高驰后，高驰可能无法正确映射其运动类型，`sportType` 字段会显示为 `65535`（16 位整型最大值，表示"未识别/未映射"）。这类活动通常：

- 距离很短（几十米）
- 没有 GPS 轨迹（无轨迹图）
- 在高驰 App 上的名称和运动类型显示正常（App 根据 `mode` 字段展示），但 API 数据中 `sportType=65535`

**这对同步没有影响**——时间重叠比对会用原始数据正确拦截，不需要额外处理。
