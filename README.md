...
> - ✅ 佳明国内国际互传：同平台，数据格式一致
> - ❌ **高驰→佳明**：高驰手表记录的运动同步到佳明后，爬升数据可能会出现明显偏差。佳明 API 返回的数据显示，高驰同步到佳明的活动 `elevationCorrected=True`，而佳明手表原生活动 `elevationCorrected=False`。从字段命名和值进行推测，佳明会对非自己手表的活动做海拔校正，结果导致高驰本来处理好的数据被佳明再加工后反而不准了。具体数值上，6.72km 徒步高驰 FIT 中原始 `total_ascent=530m`，到佳明后 `elevationGain=1891m`。
>
> **实际数据对比**：
