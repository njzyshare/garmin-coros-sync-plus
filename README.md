...
> - ✅ 佳明原生活动同步到高驰：高驰直接读取 FIT 文件中的原始爬升数据，不重新计算
> - ✅ 佳明国内国际互传：同平台，数据格式一致，佳明不做额外纠正
> - ❌ **高驰→佳明**：高驰手表记录的运动同步到佳明后，爬升数据可能会出现明显偏差。对比高驰 FIT 文件中的原始 `total_ascent`（530m）和佳明处理后的 `elevationGain`（1891m），差距达 3.57 倍。佳明对第三方上传的活动会额外标记 `elevationCorrected=True`，而佳明手表原生活动则是 `elevationCorrected=False`，说明佳明对自家设备和第三方设备做了不同处理。
>
> **实际数据对比**：
